...
parser.add_argument('--light_azimuth', default=None, type=float,
    help="Azimuth angle in degrees for key light (0-360). Overrides key_light_jitter.")
parser.add_argument('--light_elevation', default=45.0, type=float,
...
def render_scene(args,
  ...
  #     bpy.data.objects['Lamp_Fill'].location[i] += rand(args.fill_light_jitter)
    ...
    # Set key light position based on azimuth/elevation, or apply random jitter
  if args.light_azimuth is not None:
    az = math.radians(args.light_azimuth)
    el = math.radians(args.light_elevation)
    r  = args.light_radius
    bpy.data.objects['Lamp_Key'].location[0] = r * math.cos(el) * math.cos(az)
    bpy.data.objects['Lamp_Key'].location[1] = r * math.cos(el) * math.sin(az)
    bpy.data.objects['Lamp_Key'].location[2] = r * math.sin(el)
  ...
  # Save light angles to scene structure
  scene_struct['light_azimuth_deg']   = args.light_azimuth
  scene_struct['light_elevation_deg'] = args.light_elevation

  bpy.data.objects['Lamp_Fill'].data.energy = 0.0
  bpy.data.objects['Lamp_Back'].data.energy = 0.0

  bpy.data.objects['Lamp_Fill'].hide_render = True
  bpy.data.objects['Lamp_Back'].hide_render = True
  # ground = bpy.data.objects['Ground']
  # if ground.active_material is None:
  #     mat = bpy.data.materials.new(name="GroundWhite")
  #     ground.active_material = mat
  ...
  for i in range(num_objects):
    ...
    if shape_color_combos is None:
      if hasattr(args, 'shape') and args.shape:
          obj_name, obj_name_out = next(
              (s, k) for s, k in object_mapping if s == args.shape
          )
      else:
          obj_name, obj_name_out = random.choice(object_mapping)
      color_name = 'purple'
      rgba = color_name_to_rgba['purple']
    else:
    ...
    mat_name_out = 'rubber'
    mat_name = next(v for v, k in material_mapping if k == 'rubber')
