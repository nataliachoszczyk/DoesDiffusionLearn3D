...
$base_out   = "$PSScriptRoot\..\data\experiments\v42"
$clevr_data = "$PSScriptRoot\..\clevr-dataset-gen\image_generation\data"

$shape = "SmoothCube_v2"
$radius = 9
$elevation = 45
# 9 azimuth values in the requested order
$azimuths = @(0, 90, 180, 270, 360, 45, 100, 30, 135)

$idx = 0

New-Item -ItemType Directory -Force -Path $base_out | Out-Null
foreach ($a in $azimuths) {
    ...
    Write-Host "Rendering centered cube with radius=$radius, elevation=$elevation, azimuth=$a (index=$idx)"
    ...
    & $blender --background -noaudio --python $script -- `
        ...
        --filename_prefix "$shape`_r${radius}_a${a}" `
        --width 512 `
        --height 512 `
        --key_light_jitter 0 `
        --fill_light_jitter 0 `
