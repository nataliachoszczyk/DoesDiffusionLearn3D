...
$base_out   = "$PSScriptRoot\..\data\experiments\v41"
$clevr_data = "$PSScriptRoot\..\clevr-dataset-gen\image_generation\data"

$shape = "SmoothCube_v2"
$radius = 10
# 9 lamp power values from low to high
$energies = @(2, 4, 6, 8, 10, 14, 18, 24, 32)
...
foreach ($e in $energies) {
    ...
    Write-Host "Rendering centered cube with radius=$radius and light_energy=$e (index=$idx)"
    ...
    & $blender --background -noaudio --python $script -- `
        ...
        --filename_prefix "$shape`_r${radius}_e${e}" `
        --width 512 `
        --height 512 `
        --key_light_jitter 0 `
        --fill_light_jitter 0 `
