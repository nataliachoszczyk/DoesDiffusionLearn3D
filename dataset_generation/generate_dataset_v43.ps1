...
$base_out   = "$PSScriptRoot\..\data\experiments\v43"
...
for ($i = 0; $i -lt 9; $i++) {
    ...
    Write-Host "Rendering cube with radius=$radius, elevation=$elevation, azimuth=$azimuth, random position/rotation (index=$idx)"
    ...
    & $blender --background -noaudio --python $script -- `
        ...
        --filename_prefix "$shape`_r${radius}_a${azimuth}_n${idx}" `
        --width 512 `
        --height 512 `
        --key_light_jitter 0 `
        --fill_light_jitter 0 `
